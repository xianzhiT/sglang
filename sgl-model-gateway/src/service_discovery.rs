                    ...
                    match obj_res {
                        Ok(pod) => {
                            if PodInfo::should_include(&pod, &config_inner) {
                                ...
                                {
                                    let is_tracked = tracked_pods_inner
                                        .lock()
                                        .map(|tracker| tracker.contains_key(&pod_info.name))
                                        .unwrap_or_else(|e| {
                                            error!(
                                    ...
                                    if is_tracked {
                                        debug!(
                                            "Passing through tracked pod that no longer matches service discovery selectors: name={} ip={} worker_url={}",
                                            pod_info.name,
                                            pod_info.ip,
                                            pod_info.worker_url(config_inner.port)
                                        );
                                        return Some(Ok(pod));
                                    }
                                }
...
async fn handle_pod_removal(
    ...
    let (tracked_pod, remaining_count, tracked_before, tracked_after) = {
    ...
    let was_tracked = tracked_pod.is_some();

    debug!(
        "Tracked pods update after removal event: was_tracked={} | incoming=name={} ip={} worker_url={} | before={} | after={}",
        was_tracked,
        pod_info.name,
        pod_info.ip,
